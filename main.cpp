...
int main( int argc, char* args[] )
{
    ...
    character1 = IMG_LoadTexture(renderer, "down1.png");
    character2 = IMG_LoadTexture(renderer, "down1.png");
...
//    enemigos.push_back(new Enemigo(renderer));
    ...
    while(true)
    {
        ...
        {
            rect_character.y--;
            character1=IMG_LoadTexture(renderer,"up1.png");
            character2=IMG_LoadTexture(renderer,"up2.png");
        ...
        if(estado_teclas[SDL_SCANCODE_A])
        {
            if(rect_character.x>0)
                rect_character.x--;
            character1=IMG_LoadTexture(renderer,"left1.png");
            character2=IMG_LoadTexture(renderer,"left2.png");
            if(colision(rect_character,rect_character_otro))
                rect_character.x++;
        }
        ...
        if(estado_teclas[SDL_SCANCODE_S])
        {
            rect_character.y++;
            character1=IMG_LoadTexture(renderer,"down1.png");
            character2=IMG_LoadTexture(renderer,"down2.png");
            if(colision(rect_character,rect_character_otro))
                rect_character.y--;
        }
